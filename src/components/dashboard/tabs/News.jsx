...
export default function News({ darkMode }) {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h2 className={`text-base font-semibold ${textPrimary} mb-4`}>News</h2>
      ...
      >
        ...
        <div className="p-5 flex flex-col justify-center gap-2">
          ...
          <h3 className={`text-lg font-bold ${textPrimary} leading-snug`}>{featured.title}</h3>
          <p className={`text-sm ${textSecondary} leading-relaxed`}>{truncate(featured.body, 150)}</p>
          <p className={`text-xs ${textSecondary} mt-1`}>{new Date(featured.posted_at).toLocaleDateString()}</p>
        </div>
      </button>
      ...
      {rest.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {rest.map((item) => (
            ...
            >
              <div className={`h-32 ${placeholderBg} shrink-0`}>
                {item.image_url && (
      ...
      {selected && (
        <DetailModal darkMode={darkMode} onClose={() => setSelected(null)}>
          ...
          <h2 className={`text-xl font-semibold ${textPrimary} mt-2 mb-2 pr-8`}>{selected.title}</h2>
